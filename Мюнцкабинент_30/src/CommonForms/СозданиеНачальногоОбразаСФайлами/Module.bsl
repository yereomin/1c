...
Процедура ПолноеИмяФайловойБазыПриИзменении(Элемент)
	...
	Если НЕ ПустаяСтрока(СтруктураПути.Путь) Тогда
		...
		Если Не ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент()
			И МассивКаталогов.Количество() > 0 Тогда
...
&НаКлиенте
Процедура ОбработчикСохраненияФайла(Элемент,
	                                ИмяСвойства,
                                    СтандартнаяОбработка,
                                    ИмяФайла,
                                    Фильтр = "")
...
Процедура ОбработчикСохраненияФайлаПослеВыбораВДиалоге(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранныеФайлы <> Неопределено
		...
		Если ДополнительныеПараметры.Элемент = Элементы.ПолноеИмяФайловойБазы Тогда
			ПолноеИмяФайловойБазыПриИзменении(ДополнительныеПараметры.Элемент);
		КонецЕсли;
	КонецЕсли;
