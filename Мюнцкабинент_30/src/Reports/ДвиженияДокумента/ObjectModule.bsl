...
Функция ПредварительныйРасчетКоличества(Регистратор, МассивИспользуемыхРегистров)
	...
	Для Каждого ИспользуемыйРегистр Из МассивИспользуемыхРегистров Цикл
		...
		ТекстЗапросаШаблон = СтрЗаменить(ТекстЗапросаШаблон, "ВТ_ВременнаяТаблица", "ВТ_ВременнаяТаблица" + СтрЗаменить(ПолноеИмя, ".", "_"));
		
		ТекстЗапроса = ТекстЗапроса + ?(ЗначениеЗаполнено(ТекстЗапроса), ";", "") + ТекстЗапросаШаблон;
	...
	Для Каждого РезультатЗапроса Из РезультатыЗапроса Цикл
		Результат = РезультатЗапроса.Выгрузить().Получить(0);
		РассчитанноеКоличество.Вставить(Результат.ПолноеИмяРегистра, Результат.Количество);
	КонецЦикла;
...
Функция ТаблицаИспользуемыхРегистров(МассивИспользуемыхРегистров)
	...
	КолонкиТаблицыРегистров.Добавить("Сортировщик",                   Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(1, 0, ДопустимыйЗнак.Неотрицательный)));
