...
//  Коэффициенты        - Массив - коэффициенты распределения, должны быть не отрицательными;
...
Функция РаспределитьСуммуПропорциональноКоэффициентам(Знач РаспределяемаяСумма, Знач Коэффициенты, Знач Точность = 2) Экспорт
	...
	Для Индекс = 0 По Коэффициенты.Количество() - 1 Цикл
		...
		Если Коэффициент < 0 Тогда 
			// Недопустимо значение параметра Коэффициенты
			// Ожидалось: все значения коэффициентов будут неотрицательны.
			Возврат Неопределено;
		КонецЕсли;
	...
	Если СуммаКоэффициентов = 0 Тогда
		// Недопустимо значение параметра Коэффициенты
		// Ожидалось: хотя бы один коэффициент будет больше нуля.
		Возврат Неопределено;
	КонецЕсли;
...
#Иначе
	...
	ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ресурс URL: %1'"), URL);
	
	ОписаниеОшибки = ОписаниеОшибки + Символы.ПС
		+ ПредставлениеМестаДиагностики();
	...
	ЖурналПрокси = ЖурналНастроекСистемногоПрокси();
	ЖурналДиагностики = ЖурналДиагностики + Символы.ПС + ЖурналПрокси;
	
	РезультатДоступностиРесурса = ПроверитьДоступностьСервера(АдресСервераРесурса);
	ЖурналДиагностики = ЖурналДиагностики + Символы.ПС + Символы.ПС
		+ "1) " + РезультатДоступностиРесурса.ЖурналДиагностики;
	...
	Иначе 
		...
		ЖурналДиагностики = ЖурналДиагностики + Символы.ПС
			+ "2) " + РезультатКонтрольнойПроверки.ЖурналДиагностики;
	...
	ЖурналДиагностики = ОписаниеОшибки + Символы.ПС + Символы.ПС
		+ НСтр("ru = 'Результаты проверки:'") + Символы.ПС + ЖурналДиагностики;
...
#Иначе 
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Подключение проводится на компьютере <%1>.'"), ИмяКомпьютера());
#КонецЕсли
	
КонецФункции

Функция ЖурналНастроекСистемногоПрокси()
	...
	Если УказанПроксиВсехАдресов Или УказанПроксиHTTP Или УказанПроксиHTTPS Тогда 
		ЖурналПрокси = ЖурналПрокси 
			+ НСтр("ru = 'Параметры прокси-сервера: Использовать системные настройки прокси-сервера.'") + Символы.ПС 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Адрес:  %1
				           |HTTP:   %2
				           |Secure: %3
	...
	Иначе 
		ЖурналПрокси = ЖурналПрокси 
			+ НСтр("ru = 'Параметры прокси-сервера: Системные настройки прокси-сервера не заданы.'");
	КонецЕсли;
	
	Возврат ЖурналПрокси;
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	ЖурналДиагностики = Результат.ПотокВывода + Результат.ПотокОшибок;
	
	Если ЭтоWindows Тогда
		ФактНедоступности = (СтрНайти(ЖурналДиагностики, "Заданный узел недоступен") > 0)
			Или (СтрНайти(ЖурналДиагностики, "Destination host unreachable") > 0);
		
		БезПотерь = (СтрНайти(ЖурналДиагностики, "(0% потерь)") > 0)
			Или (СтрНайти(ЖурналДиагностики, "(0% less)") > 0);
	Иначе 
		ФактНедоступности = (СтрНайти(ЖурналДиагностики, "Destination Host Unreachable") > 0);
		БезПотерь = (СтрНайти(ЖурналДиагностики, "0% packet loss") > 0)
	КонецЕсли;
	
	Доступен = Не ФактНедоступности И БезПотерь;
	
	ЖурналДиагностики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Удаленный сервер %1 %2:'"), АдресСервера, ?(Доступен, НСтр("ru = 'доступен'"), НСтр("ru = 'не доступен'"))) 
		+ Символы.ПС + "> " + СтрокаКоманды + Символы.ПС + ЖурналДиагностики;
	
	Возврат Новый Структура("Доступен, ЖурналДиагностики", Доступен, ЖурналДиагностики);
...
Функция ЖурналТрассировкиМаршрутаСервера(АдресСервера)
	...
	ЖурналТрассировки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Трассировка маршрута к удаленному серверу %1:'"), АдресСервера)
		+ Символы.ПС + "> " + СтрокаКоманды + Символы.ПС
		+ Результат.ПотокВывода + Символы.ПС + Результат.ПотокОшибок;
	
	Возврат ЖурналТрассировки;
