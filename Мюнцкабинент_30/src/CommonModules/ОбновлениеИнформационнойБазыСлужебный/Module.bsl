...
Процедура ЗапуститьПроцедуруОбработкиДанных(ОбработчикОбновления, ПараметрыОбновления)
	...
	Попытка
		...
		Если ПараметрыОбновления.ПараллельныйРежим
			...
			КонецЕсли;
		Иначе
			ОбщегоНазначения.ВыполнитьМетодКонфигурации(ИмяОбработчика, ПараметрыОбработчика);
		КонецЕсли;
...
Процедура ЗапускРегистрацииДанныхОтложенногоОбновления(Параметры, АдресРезультата) Экспорт
	...
	ПараметрыОтметкиКОбработке = Неопределено;
	ОписаниеПроцедуры = Неопределено;
	Пока ЕстьСледующаяПроцедураЗаполнения(ОписаниеПроцедуры) Цикл
		ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеПроцедуры, ПараметрыОтметкиКОбработке);
	КонецЦикла;
	
	Если ПараметрыОтметкиКОбработке <> Неопределено
		И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ОбменДанными") Тогда
		МодульОбменДаннымиСервер = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСервер");
		МодульОбменДаннымиСервер.ЗавершитьЗаписьФайлаСДаннымиОбновления(ПараметрыОтметкиКОбработке);
	КонецЕсли;
	
КонецПроцедуры

Функция ЕстьСледующаяПроцедураЗаполнения(ОписаниеПроцедуры)
	
	ЕстьПроцедураЗаполнения = Ложь;
	НачатьТранзакцию();
	Попытка
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить("Константа.СведенияОбОбновленииИБ");
		...
		Для Каждого ЭлементОписаниеПроцедуры Из СведенияОбОбновлении.ОписаниеПроцедурЗаполнения Цикл
			...
			Если ОписаниеПроцедуры.Статус = "Выполнено"
				Или ОписаниеПроцедуры.Статус = "Выполняется" Тогда
				Продолжить;
			КонецЕсли;
			
			ЕстьПроцедураЗаполнения = Истина;
			ОписаниеПроцедуры.Статус = "Выполняется"; // Установка признака начала выполнения обработчика.
			ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
			Прервать;
		КонецЦикла;
		
		Если ЕстьПроцедураЗаполнения Тогда
			ЗафиксироватьТранзакцию();
		Иначе
			ОписаниеПроцедуры = Неопределено;
			ОтменитьТранзакцию();
	...
	Исключение
		...
		ВызватьИсключение;
	КонецПопытки;
	Возврат ЕстьПроцедураЗаполнения;

КонецФункции

Функция ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеПроцедуры, ПараметрыОтметкиКОбработке)
	
	Если ПараметрыОтметкиКОбработке = Неопределено Тогда
	...
	Попытка
		...
		КонецЕсли;
		НовыеСведенияОбОбновлении.ОбрабатываемыеДанные.Вставить(ОписаниеПроцедуры.ИмяОбработчика, ПараметрыОтметкиКОбработке.ДанныеОбработчика);
