		...
		Если ПравилоКонвертации.ЕстьОбработчикПриОтправкеДанных Тогда
			...
			Если СтекВыгрузки.Количество() = 1 Тогда
				...
				Для Каждого ТЧИСвойства Из ПравилоКонвертации.СвойстваТабличныхЧастей Цикл
					...
					НоваяТЧПриемника = СоздатьТЧПриемникаПоПКС(ПКСДляТЧ);
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		ИмяПланаОбмена = ЗаписьСообщения.Получатель.Метаданные().Имя;
		...
		Если ЗначениеЗаполнено(ПсевдонимПредопределенногоУзла) Тогда
			Header.NewFrom = ОбменДаннымиСервер.КодПредопределенногоУзлаПланаОбмена(ИмяПланаОбмена);
		КонецЕсли;
	Иначе
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			// Проверяем возможность увеличения версии и поддержку кодировки по УИДам в корреспонденте.
			...
			КомпонентыОбмена.Вставить("КорреспондентПоддерживаетИдентификаторОбменаДанными", Ложь);
			...
			Для Каждого ДоступнаяВерсияОтправителя Из ДоступныеВерсииОтправителя Цикл
				ДоступнаяВерсияОтправителя = СокрЛП(ДоступнаяВерсияОтправителя);
				
				// Проверяем поддерживается ли кодировка по УИДам в корреспонденте.
				Если Не КомпонентыОбмена.КорреспондентПоддерживаетИдентификаторОбменаДанными
					И СравнитьВерсии(ДоступнаяВерсияОтправителя, ПроверяемаяВерсия) >= 0 Тогда
					КомпонентыОбмена.КорреспондентПоддерживаетИдентификаторОбменаДанными = Истина;
				КонецЕсли;
				...
				Если СравнитьВерсии(ДоступнаяВерсияОтправителя, МаксимальнаяОбщаяВерсия) >= 0 Тогда
...
Функция ПоддерживаетсяВерсияСИдентификаторомОбменаДанными(Знач УзелИнформационнойБазы) Экспорт
	ПроверяемаяВерсия = НомерВерсииСПоддержкойИдентификатораОбменаДанными();
	Если УзелИнформационнойБазы.ЭтотУзел Или Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		ВерсияФорматаКорреспондента = ОписаниеВариантаНастройки.Отборы.ВерсияФорматаОбмена;
	Иначе
		ВерсияФорматаКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелИнформационнойБазы,"ВерсияФорматаОбмена");
	КонецЕсли;
	Возврат (СравнитьВерсии(ВерсияФорматаКорреспондента,ПроверяемаяВерсия) >= 0);
