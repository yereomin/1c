...
Процедура ЗапланироватьОбновлениеКонфигурации(Параметры, ПараметрыАдминистрирования)
	...
	СтрокаЗапуска = Параметры.ИмяСкриптаСозданияЗадачиПланировщикаЗадач;
	...
	ОбщегоНазначенияКлиентСервер.ЗапуститьПрограмму(СтрокаЗапуска, ПараметрыЗапускаПрограммы);
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	ФайлСкрипта.Записать(КаталогВременныхФайловОбновления + "helpers.js", КодировкаТекста.UTF8);
	...
	Если ЭтоОтложенноеОбновление Тогда 
		...
		ФайлСкрипта.Записать(ИмяСкриптаСозданияЗадачиПланировщикаЗадач, КодировкаТекста.UTF8);
